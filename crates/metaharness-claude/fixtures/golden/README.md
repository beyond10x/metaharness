...
| reviewed | before commit, for credentials and account identifiers: none present. Paths were the run's own scratch and are **redacted** to `/home/operator/...`, with derived slugs regenerated by the vendor's own rule. The `skills`, `slash_commands` and `agents` lists named what one operator had installed — 16, 47 and 5 entries — and are **neutralised** to `skill-1…`, keeping the counts and types, because the adapter is tested on carrying a list of names and never on which ones. `tools` and `capabilities` are kept: those are Claude Code's own product surface, not a description of somebody's machine. The capture's compact JSON separators are preserved deliberately — a vector mutates the exact bytes `"command":"ls"`, and re-serialising with spaces would make that mutation find nothing and pass for the wrong reason. ids are the session's own UUIDs |
