...
| reviewed | before commit, for credentials and account identifiers: none present. Paths were the run's own scratch and are **redacted** to `/home/operator/...`; derived slugs were regenerated by the vendor's own rule and the digests recomputed. The vendor's **own prompt text is also redacted** — `base_instructions` (18 KB) and seven injected developer/user blocks, each keeping its opening tag so a reader sees which block it was — because this repository is public and that text is OpenAI's rather than ours. The captured timezone went with it. The record count, their order and their types are untouched, which is what the adapter is tested on; the prompt's words are not. ids are the session's own UUIDv7s |
