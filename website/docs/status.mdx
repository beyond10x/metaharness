...
| | |
|---|---|
| **Claude Code, driven end to end** | `metaharness run claude --hermetic -p "…"` spawns 2.1.239 into a scratch config home, installs a blocking `PreToolUse` hook, answers it per call, streams protocol events on stdout, takes steering on stdin, retains the raw transcript, and exits on the hermetic floor's verdict. |
...
| **Free verbs** | `capabilities`, `conformance` and `doctor` work with **no model and no credential**. `conformance claude` runs **17** vectors; `conformance codex` runs **7**, three of which drive a real process and the real hook program. |
...
| The `allow` half of Codex's decision wire | Only the deny path has been driven. |
...
| **Q18** | `codex --version` says `0.145.0`; the same binary's `session_meta.cli_version` says `0.144.0`. The adapter keeps one pin and the reader warns rather than widening it. |
...
| # | Direction |
|---|---|
| 1 | **Contract-test every adapter mapping.** The vendor side pinned by recorded wire samples, the metaharness side by the event/command vocabulary — drift on either side a red contract rather than a surprise in a paid run. The first slice (CT-1) is built: `conformance <kind> --contract` emits a `contract_result`. |
| 2 | **More harnesses: `pi`, `opencode`** — same pattern, research record first, live proof last. |
| 3 | **metaharness usage ≡ flux usage** — UX parity with `flux run --stream-json`, and possibly a `metaharness-flux` adapter, likely the cheapest of all. |
