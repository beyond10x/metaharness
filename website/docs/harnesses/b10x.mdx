...
Pinned to **0.1.0** of `b10x-harness`. Adapter class: `direct_provider` — the loop is this family's
...
```bash
metaharness run b10x -p "…" --model-endpoint https://gateway.example --model-wire anthropic-messages
...
1. Constructs the child's environment rather than inheriting one (H3): `PATH` is
   `$HOME/.local/bin:/usr/local/bin:/usr/bin:/bin`, and nothing else arrives unnamed.
...
The contract checklist is four gaps, in words that say what stands in for each: no recorded launch
vector (the launch is unit-tested), no byte-exact replay of a captured record (the mapping is covered
by hand-written lines), no hook input — there is no hook and there will not be one — and no version
pair. `conformance b10x` therefore has nothing free to run yet.
...
Sources: `crates/metaharness-b10x/src/lib.rs`, `launch.rs` and `seam.rs`;
