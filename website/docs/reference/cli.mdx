...
| `--prompt-cache-ttl <5m\|1h>` | Main-conversation prompt cache lifetime, Claude only. Omission keeps automatic selection; other harnesses refuse an explicit value. |
...
The cache lifetime is written to this run's generated settings. Ambient cache variables remain
scrubbed; operator settings, model, effort, context, budget and tool decisions are unchanged.
The setting was observed in Claude Code **2.1.263** by static binary inspection; actual cache
behavior and cost effects remain unverified. It does not select subagent or background cache
policy. See the [design evidence and compatibility contract](https://github.com/beyond10x/metaharness/blob/main/docs/design/metaharness-protocol-v0.1.md#93-the-anti-drift-rule).
