...
```bash
metaharness conformance claude    # 17 vectors
...
tree, so H7 and H11 stop being impositions and are attested *unavailable*, naming the directory.
`--hermetic strict` therefore refuses such a run, plain `--hermetic` reports it honestly.
