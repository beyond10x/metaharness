...
|---|---|---|
| [`AEP/`](AEP/) | the AEP driven loop, through this repository's seam | `run-driven.sh` is live; `run.sh` is retired with its subject (the shell hooks) |

The subject checkout is named by `AEP_REPO` (default `~/beyond10x/aep`), and the planning plugin
checkout by `AGENTPLUGINS_REPO` (default `~/beyond10x/agentplugins`). These
