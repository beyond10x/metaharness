...
| `decomposer-ran-a-move.jsonl` | the same, plus one `aep artifact move` — `T3` |
| `plan-reviewer-clean.jsonl` | a reviewer stage that held every bound in R12 |
| `plan-reviewer-created-an-artifact.jsonl` | the same, plus one `aep artifact new` — `T4` |
...
| `plan-reviewer-no-read-verb.jsonl` | a reviewer that never ran an `aep artifact` read — `V6`/`P6` |
