...
declare_row F3 "aep plan artifact validate exits 0 inside the fixture"
...
LISTED="$(cd "$S1_FIXTURE" && aep plan artifact list --store "$STORE" --format json 2>/dev/null)"
R=0
if ! have jq; then
...
# ---- F3 -----------------------------------------------------------------------------------------
VALIDATE_OUT="$(cd "$S1_FIXTURE" && aep plan artifact validate --store "$STORE" 2>&1)" && R=0 || R=$?
...
R=0
# `aep plan artifact lifecycle story` opens with `story starts at draft`. Read, never assumed —
# which is the same discipline D3 is held to by S6.
INITIAL="$(cd "$S1_FIXTURE" && aep plan artifact lifecycle story 2>/dev/null \
  | sed -n 's/^story starts at //p' | head -1)"
if [ -z "$INITIAL" ]; then
  R=1; why "could not read the story lifecycle's initial status from \`aep plan artifact lifecycle story\`"
else
