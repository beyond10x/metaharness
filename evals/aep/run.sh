#!/usr/bin/env bash
# RETIRED, 2026-08-22 — kept as the record of what it measured, not as a thing to run.
#
# This eval's subject was the plugin's PreToolUse shell hooks: it launched one headless agent in a
# scratch config home and asked whether `store-integrity.sh` and `driven-surface.sh` held. Under
# `epic:metaharness-migration` those hooks retired: their policy is Rust inside the driver
# (`decide_tool` in protocol-cli's drive.rs), the hermetic launch is metaharness's per spawn, and
# the enforcement they carried is evaluated by `run-driven.sh` next door — which exercises the
# same deliberate-denial case through the real seam.
#
# The original script body is preserved below, unrunnable by construction, because its result
# tables are cited by the subject repository's design and plan pages and a dead link teaches less
# than a retired file.
echo "RETIRED: this eval's subject (the plugin shell hooks) no longer exists."
echo "Run ./run-driven.sh instead - the same denial case, through the metaharness seam."
exit 2
